...
define('RELEVANSSI_ALT_API', 'http://www.relevanssi.com/plugin-api/');
add_filter('pre_set_site_transient_update_plugins', 'relevanssi_alt_api_check');
add_filter('plugins_api', 'relevanssi_alt_api_information', 10, 3);
...
add_action('save_post', 'relevanssi_edit', 99);				// thanks to Brian D Gajus
...
function relevanssi_alt_api_check($transient) {
	if (empty($transient->checked))
		return $transient;

	$api_key = get_option('relevanssi_api_key', 'none');
	if ('none' == $api_key)
		return $transient;
	...
	$response = relevanssi_alt_api_request($args);
...
function relevanssi_alt_api_request($args) {
	$request = wp_remote_post(RELEVANSSI_ALT_API, array('body' => $args));
...
function relevanssi_alt_api_information($false, $action, $args) {
    $plugin_slug = plugin_basename( __FILE__ );
...
function relevanssi_edit_term($term) {
	$update = true;
	relevanssi_index_taxonomy_term($term, $update);
}
...
function _relevanssi_install() {
	...
	if(get_option('relevanssi_db_version') != $relevanssi_db_version) {
		...
		$sql = "CREATE TABLE " . $relevanssi_stopword_table . " (stopword varchar(50) NOT NULL, "
...
function relevanssi_update_log($query, $hits) {
	...
	$q = $wpdb->prepare("INSERT INTO $relevanssi_log_table (query, hits) VALUES (%s, %d)", $query, intval($hits));
	$wpdb->query($q);
}
...
function relevanssi_index_doc($indexpost, $remove_first = false, $custom_fields = false, $bypassglobalpost = false) {
	...
	foreach ($insert_data as $term => $data) {
		...
		$value = $wpdb->prepare("(%d, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %s, %s, %s, %d)",
			$post->ID, $term, $content, $title, $comment, $tag, $link, $author, $category, $excerpt, $taxonomy, $customfield, 'post', $taxonomy_detail, $customfield_detail, $mysqlcolumn);
...
<div class="postbox-container" style="width:20%; margin-top: 35px; margin-left: 15px;">
	<div class="metabox-holder">	
		...
		<div class="meta-box-sortables" style="min-height: 0">
			<div id="relevanssi_donate" class="postbox">
				<h3 class="hndle"><span>Make money with Relevanssi!</span></h3>
				<div class="inside">
					<p>Relevanssi Premium has an affiliate program.</p>
					<p>Earn <strong>50% commission</strong> on all Premium licenses you sell.</p>
					<p><strong>$25 bonus</strong> to all new affiliates.</p>
			
					<p><a href="http://www.relevanssi.com/affiliates/">Join now</a></p>
				</div>
			</div>
		</div>
...
function relevanssi_add_metaboxes() {
	add_meta_box( 
   	 ...
   	 add_meta_box(
	     'relevanssi_hidebox',
    	 __( 'Relevanssi page controls', 'relevanssi' ),
     	'relevanssi_hide_metabox',
     	'page'
   	 );
}
